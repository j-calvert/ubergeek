			...
			for (int y = templateSize / 2; y < height - (templateSize / 2); y++) {
				...
				for (int x1 = 0; x1 < templateSize; x1++) {
					for (int y1 = 0; y1 < templateSize; y1++) {
						...
						// System.out.println("valx: " + valx + " valy: " +
						// valy);
						// System.out.println("imVal: " + (input[imPos]&0xff) +
						// " convVal: " + convolveX[pos]);
					}
				}
				diffx[x + (y * width)] = (int) (valx * valx);
				diffy[x + (y * width)] = (int) (valy * valy);
				diffxy[x + (y * width)] = (int) (valx * valy);
			}
		}
...
//		double minA = 100;
		...
		for (int x = 0; x < width; x++) {
			...
			for (int y = 0; y < height; y++) {
				...
				// "  diffy:"+ diffy[y * width + x] + "  diffxy:"+ diffxy[y *
				// width + x] );
		...
		for (int x = 0; x < width; x++) {
			...
			for (int y = 0; y < height; y++) {
				...
				// val = hcr[y * width + x];
				output[y * width + x] = 0xff000000 | ((int) (val) << 16
						| (int) (val) << 8 | (int) (val));
			}
		}
